...
P3SEL0  &= ~R_FORWARD;	// R_FORWARD selected 	TB1.1		(1)
P3SEL1  &= ~R_FORWARD;	// R_FORWARD selected	TB1.1		(0)
P3OUT   &= ~R_FORWARD;	// R_FORWARD Port Pin set low		(0)
P3DIR   |= R_FORWARD;	// R_FORWARD set to Output	        (1)

P3SEL0  &= ~R_REVERSE;	// R_REVERSE selected	 TB1.2		(1)
P3SEL1  &= ~R_REVERSE;	// R_REVERSE selected	 TB1.2		(0)
P3OUT   &= ~R_REVERSE;	// R_REVERSE Port Pin set low		(0)
P3DIR   |= R_REVERSE;	// R_REVERSE set to Output		(1)

P3SEL0  &= ~L_FORWARD;	// L_FORWARD selected 	 TB2.1		(1)
P3SEL1  &= ~L_FORWARD;	// L_FORWARD selected	 TB2.1		(0)
P3OUT   &= ~L_FORWARD;	// L_FORWARD Port Pin set low		(0)
P3DIR   |= L_FORWARD;	// L_FORWARD set to Output		(1)

P3SEL0  &= ~L_REVERSE;	// L_REVERSE selected 	 TB2.2		(1)
P3SEL1  &= ~L_REVERSE;	// L_REVERSE selected	 TB2.2		(0)
...
// SET_INPUT		(0x00) // value to set port as input
//------------------------------------------------------------------------------
 P4SEL0 = SET_INPUT; // P4 set as I/0
 P4SEL1 = SET_INPUT; // P4 set as I/0
 P4DIR = SET_INPUT; // Set P4 direction to input
 
 P4SEL0 &= ~SW1; // SW1 set as I/0
 P4SEL1 &= ~SW1; // SW1 set as I/0
 P4DIR &= ~SW1; // SW1 Direction = input
 P4OUT |= SW1; // Configure pullup resistor SW1
 P4REN |= SW1; // Enable pullup resistor SW1
 ...
 P4OUT |= SW2; // Configure pullup resistor SW2
 P4REN |= SW2; // Enable pullup resistor SW2
